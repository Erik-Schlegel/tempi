...
logging.basicConfig(filename="/tempi/log/error.log", level=logging.ERROR)
...
def main():
    ...
    while not error_occurred:
        try:
            ready_to_read_file_descriptors = select.select(
                [process.stdout.fileno(), process.stderr.fileno()], [], []
            )[0]

            if process.stderr.fileno() in ready_to_read_file_descriptors:
                error_output = process.stderr.readline()
                if error_output:
                    log_error(f"Subprocess stderr: {error_output.decode('utf-8')}")
                    error_occurred = True
                    break  # Exit the loop if an error is detected

            if process.stdout.fileno() in ready_to_read_file_descriptors:
            ...
            log_error(f"Exception in main loop: {ex}")
            error_occurred = True  # Set the error flag to true to exit the loop
